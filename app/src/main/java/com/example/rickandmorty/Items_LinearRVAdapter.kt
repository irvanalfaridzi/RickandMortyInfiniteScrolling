...
class Items_LinearRVAdapter(private var itemsCells: ArrayList<Result?>) :
...
//    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder.itemViewType == Constant.VIEW_TYPE_ITEM) {
            holder.itemView.itemtextview.text = itemsCells[position]?.name
            Glide
                .with(this.mcontext)
                .load(itemsCells[position]?.image)
                .centerCrop()
